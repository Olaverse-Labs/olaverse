...
Write ``{{ olaverse_version }}`` anywhere in a Markdown page and it renders as
the version in ``olaverse/__init__.py``. The version badge on the landing page
...
PLACEHOLDER = "{{ olaverse_version }}"

_VERSION_RE = re.compile(r'^__version__\s*=\s*["\']([^"\']+)["\']', re.M)

# hooks/ sits at the repo root, alongside olaverse/.
_INIT = Path(__file__).resolve().parent.parent / "olaverse" / "__init__.py"
