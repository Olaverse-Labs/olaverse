...
def main():
    corpus_path = "/Users/olumideola/Desktop/olaverse-ai/projects/tokenizers/igbo_merged_corpus.txt"
    model_save_path = "/Users/olumideola/Desktop/olaverse-ai/olaverse/models/otk-bpe-50k-ig.json"
    
    print("Writing merged Igbo corpus to:", corpus_path)
    
    with open(corpus_path, "w", encoding="utf-8") as f_out:
        # 1. Load and stream google/WaxalNLP (ibo_tts)
        print("1. Streaming google/WaxalNLP (ibo_tts)...")
        features = Features({
            'id': Value('string'),
        ...
        try:
            waxal = load_dataset('google/WaxalNLP', 'ibo_tts', streaming=True, features=features)
            count = 0
            for item in tqdm(waxal['train'], desc="WaxalNLP (Igbo)"):
        ...
        # 2. Load and stream wikimedia/wikipedia (20231101.ig)
        print("2. Streaming wikimedia/wikipedia (20231101.ig)...")
        try:
            wiki = load_dataset('wikimedia/wikipedia', '20231101.ig', streaming=True)
            count = 0
            for item in tqdm(wiki['train'], desc="Wikipedia (Igbo)"):
                text = clean_and_normalize(item['text'])
                if text:
                    f_out.write(text + "\n")
                    count += 1
        ...
        # 3. Load and stream masakhane/masakhanews (ibo)
        print("3. Streaming masakhane/masakhanews (ibo)...")
        try:
            news = load_dataset('masakhane/masakhanews', 'ibo', streaming=True)
            count = 0
