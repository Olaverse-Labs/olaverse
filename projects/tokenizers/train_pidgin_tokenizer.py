...
def main():
    corpus_path = "/Users/olumideola/Desktop/olaverse-ai/projects/tokenizers/pidgin_merged_corpus.txt"
    model_save_path = "/Users/olumideola/Desktop/olaverse-ai/olaverse/models/otk-bpe-50k-pcm.json"
    
    print("Writing merged Pidgin corpus to:", corpus_path)
    
    with open(corpus_path, "w", encoding="utf-8") as f_out:
        # 1. Load and stream google/WaxalNLP (pcm_tts)
        print("1. Streaming google/WaxalNLP (pcm_tts)...")
        features = Features({
            'id': Value('string'),
        ...
        try:
            waxal = load_dataset('google/WaxalNLP', 'pcm_tts', streaming=True, features=features)
            count = 0
            for item in tqdm(waxal['train'], desc="WaxalNLP (Pidgin)"):
        ...
        # 2. Load and stream wikimedia/wikipedia (20231101.pcm)
        print("2. Streaming wikimedia/wikipedia (20231101.pcm)...")
        try:
            wiki = load_dataset('wikimedia/wikipedia', '20231101.pcm', streaming=True)
            count = 0
            for item in tqdm(wiki['train'], desc="Wikipedia (Pidgin)"):
                text = clean_and_normalize(item['text'])
                if text:
                    f_out.write(text + "\n")
                    count += 1
        ...
        # 3. Load and stream masakhane/masakhanews (pcm)
        print("3. Streaming masakhane/masakhanews (pcm)...")
        try:
            news = load_dataset('masakhane/masakhanews', 'pcm', streaming=True)
            count = 0
